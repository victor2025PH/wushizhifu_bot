#!/bin/bash
# 完整自動化部署腳本 - 部署到 /home/ubuntu 目錄

set -e  # Exit on error

echo "=========================================="
echo "🚀 WuShiPay 完整自動化部署 (Home 目錄版本)"
echo "=========================================="
...
PROJECT_DIR="$HOME/wushizhifu"  # 改為 home 目錄
...
# 如果當前目錄有 Bot 文件，複製過來
if [ -f "bot.py" ] || [ -f "../bot.py" ]; then
    echo -e "${YELLOW}📋 複製 Bot 代碼...${NC}"
    cp -r ../*.py bot/ 2>/dev/null || true
    cp -r ../database bot/ 2>/dev/null || true
    cp -r ../handlers bot/ 2>/dev/null || true
    cp -r ../keyboards bot/ 2>/dev/null || true
    cp -r ../middleware bot/ 2>/dev/null || true
    cp -r ../services bot/ 2>/dev/null || true
    cp -r ../utils bot/ 2>/dev/null || true
    cp -r ../config.py bot/ 2>/dev/null || true
    cp -r ../requirements.txt bot/ 2>/dev/null || true
    cp -r ../deploy bot/ 2>/dev/null || true
    if [ -f "$HOME/.env" ]; then
        cp $HOME/.env bot/.env
    fi
    echo -e "${GREEN}✅ Bot 代碼已複製${NC}"
else
    echo -e "${YELLOW}⚠️  未找到 Bot 代碼，請確保代碼已上傳${NC}"
fi
