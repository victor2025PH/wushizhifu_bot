#!/bin/bash
# 完整自動化部署腳本 - Bot + 前端 + SSL 證書

set -e  # Exit on error

echo "=========================================="
echo "🚀 WuShiPay 完整自動化部署"
echo "=========================================="
...
# 3. 設置 Bot 目錄（假設 Bot 代碼在當前用戶目錄）
echo -e "${YELLOW}🤖 步驟 3: 設置 Bot 目錄...${NC}"
if [ ! -d "bot" ]; then
    mkdir -p bot
fi

# 檢查 Bot 代碼是否在 home 目錄
if [ -f "$HOME/.env" ] && [ -f "$HOME/bot.py" ]; then
    echo -e "${YELLOW}📋 從 home 目錄複製 Bot 代碼...${NC}"
    cp -r $HOME/*.py bot/ 2>/dev/null || true
    cp -r $HOME/database bot/ 2>/dev/null || true
    cp -r $HOME/handlers bot/ 2>/dev/null || true
    cp -r $HOME/keyboards bot/ 2>/dev/null || true
    cp -r $HOME/middleware bot/ 2>/dev/null || true
    cp -r $HOME/services bot/ 2>/dev/null || true
    cp -r $HOME/utils bot/ 2>/dev/null || true
    cp -r $HOME/config.py bot/ 2>/dev/null || true
    cp -r $HOME/requirements.txt bot/ 2>/dev/null || true
    cp -r $HOME/.env bot/ 2>/dev/null || true
    echo -e "${GREEN}✅ Bot 代碼已複製${NC}"
else
    echo -e "${YELLOW}⚠️  未找到 Bot 代碼，請確保代碼在正確位置${NC}"
fi
...
if [ ! -f ".env" ]; then
    if [ -f "$HOME/.env" ]; then
        cp $HOME/.env .env
        echo -e "${GREEN}✅ 從 home 目錄複製 .env 文件${NC}"
    else
        echo -e "${RED}❌ .env 文件不存在！請創建並設置 BOT_TOKEN${NC}"
        exit 1
    fi
fi
...
# 8. 構建前端
echo -e "${YELLOW}🏗️  步驟 8: 構建前端應用...${NC}"
...
if command -v npm &> /dev/null; then
    ...
    echo -e "${YELLOW}⚠️  Node.js 未安裝，跳過前端構建${NC}"
    echo "安裝 Node.js: curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash - && sudo apt install -y nodejs"
fi

# 9. 安裝 Nginx 和 Certbot
echo -e "${YELLOW}🌐 步驟 9: 安裝 Nginx 和 Certbot...${NC}"
...
# 10. 配置 Nginx
echo -e "${YELLOW}⚙️  步驟 10: 配置 Nginx...${NC}"
...
# 12. 重啟 Nginx
echo -e "${YELLOW}🔄 步驟 12: 重啟 Nginx...${NC}"
sudo systemctl restart nginx
sudo systemctl enable nginx
echo -e "${GREEN}✅ Nginx 已啟動${NC}"

# 13. 申請 SSL 證書
echo -e "${YELLOW}🔐 步驟 13: 申請 SSL 證書...${NC}"
...
# 14. 創建 systemd 服務
echo -e "${YELLOW}⚙️  步驟 14: 創建 Bot 系統服務...${NC}"
cd ${PROJECT_DIR}/bot
